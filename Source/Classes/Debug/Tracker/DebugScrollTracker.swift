...
/**
 Scroll tracker for DEBUG mode
 - Adds debug functionalities on top of ScrollTracker
*/
class DebugScrollTracker: ScrollTracker {
    /**
     Boolean indicating whether tracking is enabled for this tracker
     */
    ...
    override init(_ view: UIScrollView,_ isNewArchEnabled: Bool = false) {
    ...
    override func updateChildren(for state: ScrollState) {
        super.updateChildren(for: state)

        if DebugHelper.selectiveTrackingEnabled, isSelected, let view: UIView = view {
            print("NowYouSeeMe: scroll state update for view: \(view), state: \(state)")
        }
    }
}
